package PrvKolokvium.TaskScheduler;
...
class PriorityTask<T> implements Task<T> {
...
class TimedTask<T> implements Task<T> {
...
class Schedulers {
    public static <T extends Task>TaskScheduler<T> getOrdered() {
        return tasks ->
            List.of(tasks).stream().sorted(Comparator.comparing(Task::getOrder)).collect(Collectors.toList());
    }

    public static <T extends Task>TaskScheduler<T> getFiltered(int order) {

        return array -> Arrays.stream(array).filter(i -> i.getOrder()<order).collect(Collectors.toList());
    }
}