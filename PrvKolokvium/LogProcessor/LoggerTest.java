package PrvKolokvium.LogProcessor;
...
interface LogProcessor<T extends ILog> {
    <T extends ILog> ArrayList processLogs(ArrayList<T> logs);
}
...
class LogSystem<T extends ILog>{
    ...
    void printResults() {
        ...
        LogProcessor<T> firstLogProcessor = new LogProcessor<T>() {
            @Override
            public <T1 extends ILog> ArrayList processLogs(ArrayList<T1> logs) {
                return (ArrayList) logs.stream().filter(i -> i.getType().equals("INFO")).collect(Collectors.toList());
            }
        };
//        LogProcessor<T> firstLogProcesso = logs -> logs.stream().filter(l -> l.getType().equals("INFO")).collect(Collectors.toCollection(ArrayList::new));
        LogProcessor<T> secondLogProcessor = new LogProcessor<T>() {
            @Override
            public <T1 extends ILog> ArrayList processLogs(ArrayList<T1> logs) {
                return (ArrayList) logs.stream().filter(i -> i.getMessage().length()<100).collect(Collectors.toList());
        ...
        LogProcessor<T> thirdLogProcessor = new LogProcessor<T>() {
            @Override
            public <T1 extends ILog> ArrayList processLogs(ArrayList<T1> logs) {
                return (ArrayList) logs.stream().sorted(Comparator.comparing(ILog::getTimestamp)).collect(Collectors.toList());
            }
        };
