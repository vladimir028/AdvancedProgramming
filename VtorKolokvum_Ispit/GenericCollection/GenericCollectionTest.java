package VtorKolokvum_Ispit.GenericCollection;
...
class GenericCollection<T extends IHasTimestamp & Comparable<T>> {
    ...
    void addGenericItem (String category, T element){
        elementMap.putIfAbsent(category, new ArrayList<>());
        elementMap.get(category).add(element);
    }
    ...
    Collection<T> itemsFromCategories (List<String> categories){
        ...
        categories.forEach(category -> {
            List<T> elements = elementMap.get(category);
            tmp.addAll(elements);
        });

        return tmp.stream().sorted(Comparator.reverseOrder()).collect(Collectors.toCollection(() -> new TreeSet<T>(Comparator.reverseOrder())));

    }

    public Map<String, Set<T>> byMonthAndDay(){
        return elementMap.values().stream().flatMap(i -> i.stream())
                .collect(Collectors.groupingBy(
                        i -> String.format("%02d-%02d", i.getTimestamp().getMonthValue(), i.getTimestamp().getDayOfMonth()),
                        TreeMap::new,
                        Collectors.collectingAndThen(
                                Collectors.toSet(),
                                set -> {
                                    TreeSet<T> tmp = new TreeSet<>(Comparator.reverseOrder());
                                    tmp.addAll(set);
                                    return tmp;
                                }
                        )
